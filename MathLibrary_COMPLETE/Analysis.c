...
double EvalFunc(int type, double x, double p[]) {
    switch(type) {case 0:return sin(x);case 1:return cos(x);case 2:return tan(x);case 3:return exp(x);case 4:if(p[2]*x+p[3]==0)return 0;return (p[0]*x+p[1])/(p[2]*x+p[3]);default:return 0;}
}
DLLEXPORT double CalculateNumericalDerivative(int type, double x, double p[]) {
    double h=0.000001; return (EvalFunc(type,x+h,p)-EvalFunc(type,x-h,p))/(2.0*h);
}
